import { CeremonyMember } from './ceremony-member.model'
...
export class Ceremony implements Saveable, Cloneable<Ceremony> {
	#id: string
	get id(): string {
		return this.#id
	}
	...
	constructor(id: string, groupId: string, name: string, members: { name: string, strikethrough: boolean }[], current: string | null) {
		...
		this.#name = name
		this.#members = members?.map(member => new CeremonyMember(member.name, member.strikethrough))
	...
	private searchMember(index: number): CeremonyMember {
	...
	reset(): void {
		this.#current = null
		this.#candidateToCurrent = null
		this.#members = this.#members.map(member => new CeremonyMember(member.name, false))
	}
	...
	static fromPrimitives(plainData: {
		...
		return new Ceremony(plainData.id, plainData.groupId, plainData.name, plainData.members, plainData.current)
	}

	clone(): Ceremony {
		return new Ceremony(this.#id, this.#groupId, this.#name, this.#members, this.#current)
	}
}
